...
let stopWalletDiscovery: (() => void) | null = null;
let stopWalletProviderSubscription: (() => void) | null = null;

function chainTokens(chainId: number): TokenInfo[] {
  return sourceChainTokens(chainId);
}

function defaultDestinationChainId(sourceChainId: number): number {
  return SUPPORTED_CHAINS.find((chain) => chain.id !== sourceChainId)?.id ?? sourceChainId;
}

function defaultSourceToken(chainId: number): TokenInfo {
...
function isSupportedWalletChain(chainId: number | null): chainId is number {
...
export const useSwapStore = defineStore("swap", {
  state: () => {
    ...
    return {
      availableWallets: [] as WalletOption[],
      ...
      walletNetworkAction: null as
        | "adding-chain"
        | "adding-route-chains"
        | "switching-route-chain"
        | null,
      walletNetworkActionChainId: null as number | null,
      quote: null as QuoteResult | null,
      allowance: 0n,
      fromBalance: 0n,
      toBalance: 0n,
      lastTxHash: null as `0x${string}` | null,
    };
  },
  getters: {
    activeWallet(state): WalletOption | null {
      if (!state.selectedWalletUuid) {
        return state.availableWallets[0] ?? null;
      }

      return (
        state.availableWallets.find(
          (wallet) => wallet.info.uuid === state.selectedWalletUuid,
        ) ?? null
      );
    },
    destinationToken(state): TokenInfo | null {
      return destinationTokenForSource(state.fromToken, state.destinationChainId);
    },
    parsedAmountIn(state): bigint {
      try {
    ...
    formattedFromBalance(state): string {
      return formatUnits(state.fromBalance, state.fromToken.decimals, 4);
    },
    formattedToBalance(state): string {
      const destinationToken = destinationTokenForSource(
        state.fromToken,
    ...
    connectedWalletName(state): string | null {
      if (!state.selectedWalletUuid) {
  ...
  actions: {
    ...
    startDiscovery(): void {
      ...
      stopWalletDiscovery = startWalletDiscovery((wallet) => {
        const existingWallet = this.availableWallets.find(
          (entry) => entry.info.uuid === wallet.info.uuid,
        );
        ...
        this.availableWallets.sort((left, right) =>
    ...
    async connectWallet(walletUuid?: string | null): Promise<void> {
      const requestedWalletUuid = walletUuid ?? this.selectedWalletUuid ?? undefined;
      const wallet =
        this.availableWallets.find((entry) => entry.info.uuid === requestedWalletUuid) ??
    ...
    async handleAccountsChanged(accounts: string[]): Promise<void> {
      ...
      this.walletAddress = accounts[0] as `0x${string}`;
      this.walletConnected = true;
      await this.refreshQuote();
    },

    async handleChainChanged(chainIdHex: string): Promise<void> {
    ...
    updateRouteSourceChain(chainId: number): void {
      ...
      const nextDestinationChainId =
        this.destinationChainId === chainId ? this.selectedChainId : this.destinationChainId;
      const nextSourceToken =
        destinationTokenForSource(this.fromToken, chainId) ?? defaultSourceToken(chainId);
    ...
    async selectRouteSourceChain(chainId: number): Promise<void> {
      this.updateRouteSourceChain(chainId);
      await this.refreshQuote();
    ...
    setSourceToken(tokenAddress: `0x${string}`): void {
      const token =
        chainTokens(this.selectedChainId).find(
          (entry) => entry.address === tokenAddress,
        ) ?? null;
    ...
    swapDirection(): void {
      ...
      const nextSourceToken =
        destinationTokenForSource(this.fromToken, nextSourceChainId) ??
    ...
    async addChainToWallet(chainId: number): Promise<void> {
    ...
    async addRouteChainsToWallet(): Promise<void> {
      if (!this.activeWallet) {
        this.status = "error";
        this.statusMessage = "Connect a wallet first";
        return;
    ...
    async switchWalletToChain(chainId: number): Promise<void> {
      if (!this.walletConnected || !this.walletAddress || !this.activeWallet) {
    ...
    async ensureSelectedChain(): Promise<void> {
      if (!this.walletConnected || !this.walletAddress || !this.activeWallet) {
        throw new Error("Connect a wallet first");
      }
    ...
    async readBalanceForToken(token: TokenInfo): Promise<bigint> {
    ...
    async refreshBalances(): Promise<void> {
      ...
      this.allowance = this.fromToken.isNative
        ...
        : await readTokenAllowance(
            this.walletAddress,
            this.fromToken,
            chainBridgeAddress(this.selectedChainId),
          );
    },
    ...
    async approve(): Promise<void> {
      ...
      try {
        ...
        const receipt = await approveBridgeToken(
          this.activeWallet.provider,
          this.walletAddress,
          this.fromToken,
          amountIn,
    ...
    async executeSwap(): Promise<void> {
      ...
      try {
        await this.ensureSelectedChain();
        const receipt = await depositToBridge(
          this.activeWallet.provider,
          this.walletAddress,
          this.selectedChainId,
          this.destinationChainId,
