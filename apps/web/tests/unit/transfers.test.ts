...
import {
  applyTransferRuntimeState,
  buildTransferRecords,
  markTransferRecordReceived,
} from "@/services/transfers";
import type { IndexedAttestation, IndexedClaim, IndexedDeposit } from "@/types/transfers";
...
describe("transfer history model", () => {
  it("marks a transfer complete when a matching claim exists", () => {
    ...
    const attestations: IndexedAttestation[] = [
      {
        id: "attestation-1",
    ...
    const [record] = buildTransferRecords(
  ...
  it("marks a transfer ready to receive when the destination root is verified", () => {
    ...
    const runtimeState = new Map<
      string,
      { claimable: boolean; sourceStateRoot: `0x${string}` | null }
    >([
      [
        "8453:1:91:0xbeef",
        {
    ...
    expect(hydratedRecord.sourceStateRoot).toBe(
  ...
  it("marks a transfer received immediately after a successful claim", () => {
    ...
    const receivedRecord = markTransferRecordReceived(
      record,
      540,
      "0xclaim-local",
    );
