...
import { readTokenAllowance, readTokenBalance, requestWalletAccounts, requestWalletChainId, } from "@/services/bridge";
import { addWalletChain, addWalletRouteChains, switchWalletChain, } from "@/services/walletChains";
vi.mock("@/services/bridge", async () => {
    const actual = await vi.importActual("@/services/bridge");
    return {
        ...actual,
...
describe("swap store", () => {
    ...
    it("connects a discovered wallet and requires approval for ERC20 deposits", async () => {
        ...
        const erc20Token = SUPPORTED_TOKENS.find((token) => token.chainId === store.selectedChainId && !token.isNative);
        if (!erc20Token) {
    ...
    it("adds a single supported chain to the connected wallet", async () => {
        ...
        expect(mockedAddWalletChain).toHaveBeenCalledWith(wallet.provider, store.destinationChainId);
    ...
    it("adds the selected route chains to the connected wallet", async () => {
        ...
        expect(mockedAddWalletRouteChains).toHaveBeenCalledWith(wallet.provider, store.selectedChainId, store.destinationChainId);
        expect(store.walletNetworkAction).toBeNull();
    });
    it("switches the connected wallet to a route chain", async () => {
        ...
        expect(mockedSwitchWalletChain).toHaveBeenCalledWith(wallet.provider, store.destinationChainId);
