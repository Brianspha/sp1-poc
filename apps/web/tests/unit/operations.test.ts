...
describe("operations dashboard service", () => {
  ...
  it("normalizes node-manager and indexer responses", async () => {
    ...
    expect(dashboard.runtime.totalBaseStakeAmount).toBe(1_000_000_000_000_000_000_000n);
    expect(dashboard.runtime.validators[0]?.pendingRewards).toBe(1_000_000_000_000_000_000n);
    expect(dashboard.runtime.rewardTokenSymbol).toBe("BBTA");
    expect(dashboard.runtime.rewardReserveBalance).toBe(25_000_000_000_000_000_000n);
  ...
  it("submits reward reserve top-up requests", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:7011/rewards/top-up",
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({ amountWei: "1000000000000000000" }),
      }),
    );
    expect(result.topUpTransactionHash).toBe(
      "0x2222222222222222222222222222222222222222222222222222222222222222",
    );
  });
});
